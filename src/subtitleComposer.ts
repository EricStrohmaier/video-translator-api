...
function hexToAss(hex: string | undefined, defaultHex?: string): string {
  ...
  const aaASS = (255 - parseInt(aaRGBA, 16))
    .toString(16)
    .toUpperCase()
    .padStart(2, "0");
...
export type SubtitleOptions = {
  ...
  // Request-configurable padding and width
  padX?: number; // horizontal padding
  padTop?: number; // top padding
  padBottom?: number; // bottom padding
  maxWidthFraction?: number; // fraction of video width used for text block (0.5..0.98)
};

async function ensureExternalFont(
  fontUrl: string,
  fontsDir: string
): Promise<{ fname: string; fpath: string } | null> {
  ...
  try {
    if (await isLikelyFont(fpath)) needDownload = false;
  } catch {}
  if (needDownload) {
...
function mergeFrameTexts(
  frames: { frameNumber: number; text: string }[],
  fps = 1
) {
 ...
 */
export async function overlaySubtitlesBottomCenter(
  inputPath: string,
  translatedFrames: TranslatedFrame[],
  outputPath: string,
  ...
  const vStream = Array.isArray(meta?.streams)
    ? meta.streams.find((s: any) => s.codec_type === "video")
    : null;
  ...
  const roundedRadius = Math.max(
    0,
    Number(opts.roundedRadius ?? (process.env.ASS_ROUNDED_RADIUS ?? 12))
  );
  const bgBlur = Math.max(0, Number(opts.bgBlur ?? 0));
  const useRounded = roundedRadius > 0; // simple: rounded corners without blur

  const padX = (() => {
    const v = (opts as any)?.padX;
    if (typeof v === 'number' && isFinite(v)) return Math.max(1, Math.round(v));
    const envV = Number((process.env.ASS_PAD_X ?? '').toString());
    if (isFinite(envV) && envV > 0) return Math.round(envV);
    return Math.round(boxPad * 0.75);
  })();
  const padTop = (() => {
    const v = (opts as any)?.padTop;
    if (typeof v === 'number' && isFinite(v)) return Math.max(1, Math.round(v));
    const envV = Number((process.env.ASS_PAD_TOP ?? '').toString());
    if (isFinite(envV) && envV > 0) return Math.round(envV);
    return Math.round(boxPad * 0.6);
  })();
  const padBottom = (() => {
    const v = (opts as any)?.padBottom;
    if (typeof v === 'number' && isFinite(v)) return Math.max(1, Math.round(v));
    const envV = Number((process.env.ASS_PAD_BOTTOM ?? '').toString());
    if (isFinite(envV) && envV > 0) return Math.round(envV);
    return boxPad;
  })();
  const maxWidthFraction = (() => {
    const v = (opts as any)?.maxWidthFraction;
    const clamp = (n: number) => Math.min(0.98, Math.max(0.5, n));
    if (typeof v === 'number' && isFinite(v)) return clamp(v);
    const envV = Number((process.env.ASS_MAX_WIDTH_FRACTION ?? '').toString());
    if (isFinite(envV) && envV > 0) return clamp(envV);
    return 0.9;
  })();
  ...
  const decided = perFrame.map((pf) => {
    ...
    const maxWidth = Math.floor(vW * maxWidthFraction) - padX * 2;
    let out = s;
    if (!forceOneLine) {
      ...
      if (cjk) {
        ...
        for (const ch of tokens) {
          const cand = line + ch;
          const est = cand.length * factor * baseFontSize;
          if (est > maxWidth && line) {
            lines.push(line);
            line = ch;
          } else {
            line = cand;
        ...
        if (line) lines.push(line);
        if (lines.length > 0) lines[0] = (prefix + lines[0]).trim();
        out = lines.join("\n");
      }
    }
  ...
  for (const ev of merged) {
    ...
    const contentH = lineHeights.reduce(
      (acc, h, i) => acc + h + (i ? lineGap : 0),
      0
    );
    const estWidths = lines.map(
      (ln) => ln.length * (isCJK(ln) ? cjkFactor : latinFactor) * baseFontSize
    );
    const contentW = Math.max(1, Math.round(Math.max(...estWidths)));
    const boxW = Math.round(contentW + padX * 2);
    const boxH = Math.round(contentH + padTop + padBottom);
    ...
    if (useRounded) {
      ...
      const baselineOffset = Math.round(baseFontSize * 0.15);
      ...
      events.push(
        `Dialogue: 0,${start},${end},Default,,0,0,0,,${shapeDialogue}`
      );
      events.push(
        `Dialogue: 1,${start},${end},Default,,0,0,0,,${textDialogue}`
      );
    } else {
  ...
  if (
    (opts.fontUrl && opts.fontUrl.trim().length > 0) ||
    (ASS_FONT_URL && ASS_FONT_URL.trim().length > 0)
  ) {
    const fontsDir = path.join(outDir, "fonts");
    try {
      const info = await ensureExternalFont(
        opts.fontUrl && opts.fontUrl.trim().length > 0
          ? opts.fontUrl
          : ASS_FONT_URL,
        fontsDir
      ...
      if (info) {
        const fontsDirEsc = fontsDir
          .replace(/\\/g, "\\\\")
          .replace(/:/g, "\\:")
          .replace(/'/g, "\\'");
        const chosenName =
          opts.fontName && opts.fontName.trim().length > 0
            ? opts.fontName
            : ASS_FONT_NAME && ASS_FONT_NAME.trim().length > 0
            ? ASS_FONT_NAME
            : deriveFontNameFromFilename(info.fname);
        ...
        if (DEBUG_COMPOSER)
          console.log(
            `   [debug] Using external font '${chosenName}' from ${info.fname}`
          );
      }
    } catch (e: any) {
      if (DEBUG_COMPOSER)
        console.log(`   [warn] External font failed: ${e.message}`);
    }
  }
  const filterComplex = `scale=trunc(iw/2)*2:trunc(ih/2)*2,${subFilter}`;

  const run = (videoCodec: "h264_videotoolbox" | "libx264") =>
    new Promise<void>((resolve, reject) => {
      ...
      cmd
        .on("start", (cl: string) => {
          if (DEBUG_COMPOSER) console.log(`   [debug] FFmpeg command: ${cl}`);
        })
        .on("stderr", (line: string) => {
          if (DEBUG_COMPOSER) console.log(`   [ffmpeg] ${line}`);
        })
        .on("end", () => resolve())
...
export async function renderSubtitlesPreview(
  inputPath: string,
  frame: TranslatedFrame,
  outputPngPath: string,
  opts: SubtitleOptions = {}
) {
  const tempVideoOut = path.join(
    path.dirname(outputPngPath),
    "preview_tmp_video.mp4"
  );
  // If previewAtSeconds is provided, trim the source to a 1s clip starting there
  const startAt =
    typeof (opts as any)?.previewAtSeconds === "number" &&
    (opts as any).previewAtSeconds >= 0
